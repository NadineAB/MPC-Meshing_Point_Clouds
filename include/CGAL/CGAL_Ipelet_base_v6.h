...
//
// $URL: svn+ssh://scm.gforge.inria.fr/svn/cgal/branches/releases/CGAL-4.1-branch/CGAL_ipelets/include/CGAL/CGAL_Ipelet_base_v6.h $
// $Id: CGAL_Ipelet_base_v6.h 70786 2012-07-27 10:29:10Z sloriot $
// 
//
...
public:    
    ...
    {
      IpeSegmentSubPath* SSP_ipe = new IpeSegmentSubPath;
      IpeVector ipeS=IpeVector( CGAL::to_double(CGAL::cpp0x::get<1>(arc).x()),
                                CGAL::to_double(CGAL::cpp0x::get<1>(arc).y()));//convert ot ipe format
      IpeVector ipeT=IpeVector( CGAL::to_double(CGAL::cpp0x::get<2>(arc).x()),
                                CGAL::to_double(CGAL::cpp0x::get<2>(arc).y()));//convert ot ipe format
      SSP_ipe->AppendArc(IpeMatrix(sqrt(CGAL::to_double(CGAL::cpp0x::get<0>(arc).squared_radius())),0,
