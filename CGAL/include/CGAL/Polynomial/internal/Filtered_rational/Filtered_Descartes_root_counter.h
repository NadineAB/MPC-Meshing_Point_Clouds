...
//
// $URL: svn+ssh://scm.gforge.inria.fr/svn/cgal/branches/releases/CGAL-4.1-branch/Kinetic_data_structures/include/CGAL/Polynomial/internal/Filtered_rational/Filtered_Descartes_root_counter.h $
// $Id: Filtered_Descartes_root_counter.h 67093 2012-01-13 11:22:39Z lrineau $
// 
//
...
//std::cerr << "Why are you solving constant function?\n";
        ...
        bi= k.exact_to_interval_converter_object().nt_converter()(begin);
        ei= k.exact_to_interval_converter_object().nt_converter()(end);

//! Could add their optimization here if shift is positive
        fim = k.interval_traits_object().map_rational_interval_to_positive_object(fh.interval_function())( bi, ei);
...
#ifndef NDEBUG
        {
                                                  //= efc.nt_converter()(begin);
            typename Kernel::Exact_traits::NT be= begin;
                                                  //= efc.nt_converter()(end);
            typename Kernel::Exact_traits::NT ee=end;
            typename Kernel::Exact_traits ek=k.exact_traits_object();
            ...
            CGAL_exactness_assertion(ac.number_of_alternations() == ace.number_of_alternations());
        }
...
//typename Kernel::Exact_to_interval_converter efc= k.exact_to_interval_converter_object();
                                                  //= efc.nt_converter()(begin);
        typename Kernel::Exact_traits::NT be= begin;
        typename Kernel::Exact_traits::NT ee=end; //= efc.nt_converter()(end);
        typename Kernel::Exact_traits ek=k.exact_traits_object();
...
class Filtered_Descartes_root_counter
{
    public:
        ...
        {
            result_type ct=  filtered_Descartes_root_counter(h_, begin, end, true, kernel_);
//std::cout << h_ << " has a root bound of " << ct << " in " << begin << " to " << end << std::endl;
            return ct;
        }
